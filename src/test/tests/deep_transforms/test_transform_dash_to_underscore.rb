...
class TestTransformDashToUnderscore < BaseTest
  ...
  def test_array_transform_shallow
    initial_value = [{'key-name1' => 1, 'key-name2' => 2}, {'key-name3' => 3, 'key-name4' => 4}]
  ...
  def test_array_transform_deep
    initial_value = [[[[[{'key-name1' => 1}, {'key-name2' => [[[{'key-name3' => 3}]]]}]]]]]
  ...
  def test_hash_transform_shallow
    initial_value = {'key-name1' => 1, 'key-name2' => 2}
  ...
  def test_hash_transform_deep
    initial_value = {'key-name1' => 1, 'key-name2' => {'key-name3' => {'key-name4' => {'key-name5' => 5}}}}
  ...
  def test_complex_transform_deep
    initial_value = {'key-name1' => 1, 'key-name2' => [{'key-name3' => [{'key-name4' => 4, 'key-name5' => 5}]}]}
  ...
  def assert_initial_value_is_transformed_to_expected_value(initial_value, expected_value)
    assert_equal expected_value, HashKeyTransformer.transform_dash_to_underscore(initial_value)
  end

  def assert_transformed_value_is_equal(value)
    assert_equal value, HashKeyTransformer.transform_dash_to_underscore(value)
  end
